...
//echo ' -- ' .$this->item->title . ' : ' . $parent_submenu . '<br/>';
	...
	function get_submenu_wrap_start(){
		...
		*/
		...
		//Retractor
		if( $this->drop_sub && ( ubermenu_op( 'display_retractor_top' , $this->args->uber_instance ) == 'on' ) ){
			$retractor_tag = $this->submenu_tag == 'ul' ? 'li' : 'div';
			$item_output.= '<'.$retractor_tag.' class="ubermenu-retractor ubermenu-retractor-mobile"><i class="fa fa-times"></i> '.__( 'Close' , 'ubermenu' ).'</'.$retractor_tag.'>';
		}

		if( $this->drop_sub && ( ubermenu_op( 'display_submenu_close_button' , $this->args->uber_instance ) == 'on' ) ){
			$retractor_tag = $this->submenu_tag == 'ul' ? 'li' : 'div';
	...
	function get_submenu_wrap_end(){
		...
		//Retractor
		if( $this->drop_sub && ( ubermenu_op( 'display_retractor_bottom' , $this->args->uber_instance ) == 'on' ) ){
			$retractor_tag = $this->submenu_tag == 'ul' ? 'li' : 'div';
			$html.= '<'.$retractor_tag.' class="ubermenu-retractor ubermenu-retractor-mobile"><i class="fa fa-times"></i> '.__( 'Close' , 'ubermenu' ).'</'.$retractor_tag.'>';
		}
	...
	function get_anchor( $atts ){
		...
		//Get custom pieces 
		$custom_pieces = array();
		extract( apply_filters( 'ubermenu_custom_item_layout_data' , $custom_pieces , $layout , $this->ID ) );
