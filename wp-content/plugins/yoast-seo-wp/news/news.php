...
// News Posttype
add_action( 'init', 'hjemmesider_news_create_posttype2' );
	function hjemmesider_news_create_posttype2() {
...
function hjemmesider_news_function2() {
	hjemmesider_news_create_posttype2();
	flush_rewrite_rules();
}
register_activation_hook( __FILE__, 'hjemmesider_news_function2' );


/* -------------------------------------- */

add_action( 'init', 'create_simplenews_hierarchical_taxonomy2', 0 );

//create a custom taxonomy name it topics for your posts

function create_simplenews_hierarchical_taxonomy2() {
...
// Change author
function simple_news_allowAuthorEditing2() {
  add_post_type_support( 'news', 'author' );
}
add_action('init','simple_news_allowAuthorEditing2');
...
// CSS file
add_action('wp_enqueue_scripts', 'hjemmesider_news_register_plugin_styles2');
  function hjemmesider_news_register_plugin_styles2() {
...
if ( isset( $options['theme_files']  )) {
	...
	if ( !file_exists( trailingslashit( get_stylesheet_directory() ) . 'archive-news.php' ) || !file_exists(trailingslashit( get_template_directory() ) . 'archive-news.php' )  ) {
		add_filter( 'template_include', 'simple_news_page_template2' );
		function simple_news_page_template2( $template ) {
			$new_template = null;
