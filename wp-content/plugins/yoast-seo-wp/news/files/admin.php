<?php
add_action( 'admin_menu', 'simple_news_add_admin_menu2' );
add_action( 'admin_init', 'simple_news_settings_init2' );

function simple_news_add_admin_menu2(  ) {

    add_options_page( 'Simple News', 'Simple News', 'manage_options', 'simple_news', 'simple_news_options_page2' );

}

function simple_news_settings_init2(  ) {

    register_setting( 'pluginPage', 'simple_news_settings' );

    add_settings_section(
        'simple_news_pluginPage_section2',
        __( 'Settings:', 'simple-news' ),
        'simple_news_settings_section_callback2',
        'pluginPage'
    );

    add_settings_section(
        'simple_news_settings_section_info2',
        __( 'Settings:', 'simple-news' ),
        'simple_news_settings_section_info2',
        'pluginPage'
    );
    ...
    add_settings_field(
        'simple_news_select_field_02',
        __( 'Image size', 'simple-news' ),
        'simple_news_select_field_0_render2',
        'pluginPage',
        'simple_news_pluginPage_section2'
    );
    ...
    add_settings_field(
        ...
        'simple_news_theme_files_render2',
        'pluginPage',
        'simple_news_pluginPage_section2'
    );
...
function simple_news_select_field_0_render2(  ) {

    $options = get_option( 'simple_news_settings' );
    ?>
    <select name='simple_news_settings[simple_news_select_field_02]'>
        <option value='1' <?php selected( $options['simple_news_select_field_02'], 1 ); ?>>ImageSize 700 x 700 - Default</option>
        <option value='2' <?php selected( $options['simple_news_select_field_02'], 2 ); ?>>ImageSize - Original</option>
    </select>

<?php }


function simple_news_theme_files_render2(  ) {
...
function simple_news_settings_section_callback2(  ) {
    echo '<p class="simple_news_info">' . __( 'Choose between 2 image sizes', 'simple-news' ) . '</p>';
}

function simple_news_settings_section_info2(  ) {
...
function simple_news_options_page2(  ) { ?>
    <style>
